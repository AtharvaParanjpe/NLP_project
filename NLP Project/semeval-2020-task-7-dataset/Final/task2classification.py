...
class Subtask1_Model(tf.keras.Model):
    def __init__(self):
        ...
        self.bert_layer = TFBertForSequenceClassification.from_pretrained('bert-base-uncased', num_labels=3)
...
#         self.dense_layer1 = tf.keras.layers.Dense(3, activation="sigmoid",dtype='float32')
...
#         output2 = self.dense_layer1(output1)
        ...
        # output2 = self.dense_layer1(output1)
        # output3 = self.soft_max(output2)
        return self.bert_layer(x_train)
...
def training(train_data):
    ...
    with tf.GradientTape() as tape:
        ...
        input_dictionary['training'] = True
        targets = tf.convert_to_tensor(np.array(targets), dtype="float32")[None, :]
        # targets = tf.reshape(targets ,shape=(targets.shape[0],1))
        input_dictionary['labels'] = targets
    ...
    for index, row in train_data.iterrows():
        in_id1 = tokenizer.encode(row['edit1'], row["original"], add_special_tokens=True)
        attn1,seg1 = get_bert_params(tokenizer.convert_ids_to_tokens(in_id1))
        in_id1 += [0]*(maxLengthPadding-len(in_id1))
        input_ids.append(in_id1)
        attn_mask.append(attn1)
    ...
    for index, row in train_data.iterrows():
        in_id1 = tokenizer.encode(row['edit2'], row["original"], add_special_tokens=True)
        attn1,seg1 = get_bert_params(tokenizer.convert_ids_to_tokens(in_id1))
        in_id1 += [0]*(maxLengthPadding-len(in_id1))
        input_ids.append(in_id1)
        attn_mask.append(attn1)
...
def testing(test_data):
    ...
    for index, row in test_data.iterrows():
        ...
        targets = tf.convert_to_tensor(np.array(row["label"]), dtype="float32")
        # targets = tf.reshape(targets ,shape=(targets.shape[0],1))
        input_dictionary['labels'] = targets
        ...
        logits_out = output.logits[0]
        # print(logits_out)
        output = numpy.where(logits_out == numpy.amax(logits_out))[0][0]
                
        if(output==row['label']):
            correct+=1

        if(index<50):
          print(logits_out, output, row['label'])
