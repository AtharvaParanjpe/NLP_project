...
train = pd.read_csv(path + "subtask-1/modified-train.csv")
train = train.append(dev, ignore_index=False)
test = pd.read_csv(path + "subtask-1/modified-test.csv")
...
def training(train_data):
    ...
    for index, row in train_data.iterrows():
        ...
        in_id1 = tokenizer.encode(row['edit'], add_special_tokens=True)
        attn1,seg1 = get_bert_params(tokenizer.convert_ids_to_tokens(in_id1))
        in_id1 += [0]*(maxLengthPadding-len(in_id1))
...
def testing(test_data):
    ...
    for index, row in test_data.iterrows():
        ...
        loss+= (output1*3-row["meanGrade"])**2
